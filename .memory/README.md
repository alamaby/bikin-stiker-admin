...
Last updated: 2026-09-13 sore (WIB)
Format version: 1
...
- Migrasi utama ter-commit + push (`adc2611`). Verifikasi lanjutan (mobile + EN + search) selesai; 2 file berubah menunggu commit: `src/layout/AppSidebar.tsx`, `src/app/[locale]/layout.tsx` (+ plan).
- `npm run lint` bersih; `npm run build` final menyusul sebelum commit.
