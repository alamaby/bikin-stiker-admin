"use client";
// Adapted from TailAdmin free-nextjs-admin-dashboard AppHeader (MIT License).
// Changes: locale-aware search redirect to filters, compact LocaleSwitcher,
// ThemeToggleButton on local ThemeContext, real NotificationDropdown + UserDropdown.
...
export default function AppHeader({
  ...
  function handleSearchSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!q) return;
    // Redirect to the most relevant filter: llm-logs full-text search.
    router.push(`${buildLocaleHref(locale, "/llm-logs")}?q=${encodeURIComponent(q)}`);
    ...
    <header className="sticky top-0 z-99999 flex w-full border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 lg:border-b">
      <div className="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
        <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          <button
            className="z-99999 flex h-10 w-10 items-center justify-center rounded-lg border-gray-200 text-gray-500 dark:border-gray-800 dark:text-gray-400 lg:flex lg:h-11 lg:w-11 lg:border"
          ...
          <Link href={buildLocaleHref(locale, "/")} className="lg:hidden">
            <span className="text-base font-bold text-gray-900 dark:text-white">Bikin Stiker Admin</span>
          </Link>
        ...
        >
          <div className="flex items-center gap-2 2xsm:gap-3">
            ...
            <ThemeToggleButton />
            <NotificationDropdown items={notifications} locale={locale} />
          </div>
          <UserDropdown userEmail={userEmail} locale={locale} />
        </div>
      </div>
    </header>
  );
